    ...
    <div class="row">
            ...
            <div class="card">
                <ul class="list-group flex-column">
                    ...
                    <li
                        ...
                        Lawn size ({{ $order->lawnSize->name }})<span class="">${{ $order->lawn_size_amount }}</span></li>
                    <li
                        class="list-group-item d-flex justify-content-between align-items-center py-4 px-4">
                        Lawn height ({{ $order->lawnHeight->name }})<span class="">${{ $order->lawn_height_amount }}</span></li>
                    @if ($order->cornerLot)
                        <li class="list-group-item d-flex justify-content-between align-items-center py-4 px-4">
                    ...
                    @if ($order->fence)
                        <li class="list-group-item d-flex justify-content-between align-items-center py-4 px-4">
                        Fence ({{ $order->fence->name }}) <span class="">${{ $order->fence_amount }}</span></li>
                    @endif
                    @if ($order->cleanUp)
                        <li class="list-group-item d-flex justify-content-between align-items-center py-4 px-4">
                            Yard cleanup ({{ $order->cleanUp->name }})<span class="">${{ $order->cleanup_amount }}</span></li>
                    @endif
                    <li
                        class="list-group-item d-flex justify-content-between align-items-center py-4 px-4">
        ...
        <div class="col-lg-6 col-md-6 col-sm-12 col-12 d-flex flex-column justify-content-between">
                    ...
                    <div class="row mt-3 {{ $order->tip && $order->service_type == 2 ?: 'd-none' }}">
                        <div class="col-md-12">
                            <div class="form-check form-check-inline m-t-10">
                                <input class="form-check-input" type="radio" id="one-time-tip" name="tip_type" value="one-time" {{ $order->tip_type ? $order->tip_type == 'one-time' ? 'checked' : '' : 'checked' }}>
                                <label for="one-time-tip">Add this tip percentage only on first order</label>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-check form-check-inline m-t-10">
                                <input class="form-check-input" type="radio" id="recurring-tip" name="tip_type" value="recurring" {{ $order->tip_type == 'recurring' ? 'checked' : '' }}>
            ...
            <div class="text-end">
                ...
                @else
                    <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modal-opener" data-title="Add Card" data-url="{{ route('payments.add-card-form',['order_id'=>$order->id,'type'=>$type ?? '','service' => 'lawn-mowing']) }}">Add Card</button>
                @endif
